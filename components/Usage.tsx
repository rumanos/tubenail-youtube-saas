...
function Usage({ featureFlag, title }: { featureFlag: FeatureFlag, title: string }) {
  ...
  if (isPending) {
    return <div className="text-zinc-500 text-center py-4">Loading...</div>
  }
    ...
    return (
      <div className="bg-white rounded-2xl shadow-sm border border-red-100 p-6">
        ...
        <div className="relative">
          <Progress
            value={100}
            className="h-3 rounded-full bg-zinc-100 [&>*]:bg-red-600"
          />
    ...
    return (
      <div className="bg-white rounded-2xl shadow-sm border border-red-200 p-6">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-semibold text-zinc-800">{title}</h2>
          <div className="px-4 py-2 bg-red-50 rounded-lg">
            <span className="text-red-800 text-sm">Feature Disabled</span>
          </div>
        </div>

        <div className="relative">
          {/* <Progress value={0} className="h-3 rounded-full bg-zinc-100" /> */}
          <p className="text-sm text-zinc-500 mt-2">Upgrade to use this feature</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-4 gap-4">
        <h2 className="text-xl font-semibold text-zinc-800">{title}</h2>
        <div className="px-4 py-2 bg-zinc-50 rounded-lg">
          <span className="font-medium text-zinc-700">{featureUsage}</span>
          <span className="text-zinc-400 mx-2">/</span>
          <span className="font-medium text-zinc-700">{featureAllocation}</span>
        </div>
      </div>
      ...
      <div className="relative">
        <Progress
          value={progress}
          className={`h-3 rounded-full bg-zinc-100 ${progressColor}`}
        />
