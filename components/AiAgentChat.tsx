...
function AiAgentChat({ videoId }: { videoId: string }) {
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="hidden lg:block px-4 pb-3 border-b border-transparent">
        <h2 className="text-lg font-semibold text-zinc-800">AI Agent</h2>
      </div>

      {/* Messages */}
      <div className="flex-1 overflow-y-auto px-4 py-4 bg-zinc-50 rounded-4xl" ref={messagesContainerRef}>
        <div className="space-y-6">
          ...
          {messages.map((m) => (
            <div key={m.id} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
              <div className={`max-w-[85%] ${m.role === "user" ? "bg-blue-500" : "bg-white"} rounded-md drop-shadow-xs px-4 py-3`}>
                  ...
                  <div className="space-y-3">
                    {m.parts.map((part, i) =>
                      part.type === 'text' ? (
                        ...
                        <div
                          key={i}
                          className="bg-white/50 rounded-lg p02 space-y-2 text-zinc-800"
                        >
      ...
      <div className="border-t border-transparent p-4">
        <div className="space-y-3">
          <form onSubmit={handleSubmit} className="flex gap-2">
            <input
              className="flex-1 px-4 py-2 text-sm border border-zinc-200 rounded-full focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              type="text"
              placeholder={
          ...
          <div className="flex gap-2">
            <button
              className="text-xs xl:text-sm w-full flex items-center justify-center gap-2 py-2 px-4 bg-zinc-100 hover:bg-zinc-200 rounded-full transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
